...
import { CERUSD } from 'constants/tokens'
...
export function useUniContract() {
  ...
  const cerUSDAddress = useMemo(() => (chainId ? CERUSD?.address : undefined), [chainId])
  return useContract(cerUSDAddress, UNI_ABI, true)
}
    ...
    return {
      data: proposalsCallData.map((proposal, i) => {
        ...
        return {
          ...
          forCount: CurrencyAmount.fromRawAmount(CERUSD, proposal?.result?.forVotes),
          againstCount: CurrencyAmount.fromRawAmount(CERUSD, proposal?.result?.againstVotes),
          startBlock,
...
export function useQuorum(governorIndex: number): CurrencyAmount<Token> | undefined {
  ...
  return CurrencyAmount.fromRawAmount(CERUSD, quorumVotes)
}
...
export function useUserVotes(): { loading: boolean; votes: CurrencyAmount<Token> | undefined } {
  ...
  return useMemo(() => {
    return { loading, votes: CERUSD && result ? CurrencyAmount.fromRawAmount(CERUSD, result?.[0]) : undefined }
...
export function useUserVotesAsOfBlock(block: number | undefined): CurrencyAmount<Token> | undefined {
  ...
  const votes = useSingleCallResult(uniContract, 'getPriorVotes', [account ?? undefined, block ?? undefined])
    ?.result?.[0]
  return votes && CERUSD ? CurrencyAmount.fromRawAmount(CERUSD, votes) : undefined
}
...
export function useProposalThreshold(): CurrencyAmount<Token> | undefined {
  ...
  if (res?.result?.[0] && CERUSD) {
    return CurrencyAmount.fromRawAmount(CERUSD, res.result[0])
  }
