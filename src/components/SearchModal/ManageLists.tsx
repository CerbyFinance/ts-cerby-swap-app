...
const ListRow = memo(function ListRow({ listUrl }: { listUrl: string }) {
  ...
  const activeTokensOnThisChain = useMemo(() => {
    ...
    // @ts-ignore
    return list.pools.reduce((acc, cur) => (cur.chainId === chainId ? acc + 1 : acc), 0)
  }, [chainId, list])
...
export function ManageLists({
  ...
  setImportList: (list: PoolList) => void
  ...
  const [tempList, setTempList] = useState<PoolList>()
              ...
              <RowFixed>
                ...
                <AutoColumn gap="4px" style={{ marginLeft: '20px' }}>
                  ...
                  <ThemedText.Main fontSize={'12px'}>
                    <Trans>{tempList.pools.length} tokens</Trans>
                  </ThemedText.Main>
                </AutoColumn>
              </RowFixed>
              {isImported ? (
